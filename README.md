...
docker run -it -v C:\Code\ops\migration:/migration -w="/migration" contentful-migration:latest autoMigrateContentType.ts --space-id=xxxxxx --environment-id=dev --id=myNewType --management-token=xxxx-xxxxx-xxxxx--xxxxxxxxxxxxxxxxxxxxxxx

Apply the new content type "my-new-type" in the target space staging
docker run -it -v C:\Code\ops\migration:/migration -w="/ops/migration" contentful-migration:latest ts-node dfdsMigrate.ts --space-id=xxxxxx --environment-id=staging --management-token=xxxx-xxxxx-xxxxx--xxxxxxxxxxxxxxxxxxxxxxx
